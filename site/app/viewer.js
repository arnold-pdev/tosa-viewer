// 3D view: render window, VTP loading, diverging LUT, scalar bar.
...
export function displayRange(scalar) {
  // Diverging fields get a range symmetric about 0 (white exactly at 0).
  if (scalar.diverging) {
...
export function createViewer(container) {
  ...
  async function showSurface(arrayBuffer, scalar) {
    ...
    // Ambient-heavy lighting: vtk.js does not two-side-light the scalar
    // texture path, so staircase faces pointing away from the headlight would
    // otherwise render black. High ambient keeps the colormap readable
    // everywhere; diffuse adds depth cues on lit faces.
